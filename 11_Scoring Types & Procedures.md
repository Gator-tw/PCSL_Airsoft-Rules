...
Affiliated PCSL Shotgun or PCSL 3-Gun matches may use either Hit Factor (defined under [Section 11.2](#id-11.2-hit-factor-scoring-type) - [Section 11.3](#id-11.3-hit-factor-target-and-penalty-values)) or Time-Plus Points scoring (defined under [Section 11.10](#id-11.10-time-plus-points-scoring-type) - [Section 11.11](#id-11.11-time-plus-points-neutralization-criteria-and-penalties)).
...
Misses (“Mikes”) are worth minus 10 points (-10). If a competitor has fewer than the required amount of hits on a target, they will be assigned one miss for each missing hit.
...
Procedural penalties are worth minus 10 points (-10) per occurrence. Reference [Chapter 12](12_Procedural%20Penalties.md).
...
1. Be painted prior to every competitor’s run
2. Shall be required to have a “Hit!” call from an RO
3. Be targets that clearly visually or audibly indicate a hit to the competitor. These targets must be identified in the WSB.
...
Some static steel arrays may be shot more quickly than accurate hit calls can be verbally made by the ROs. In these cases, it’s recommended to use the verbal indication “Good!” ([Section 5.1.12](<05_Range Commands.md#id-5.1.12-supplemental-range-commands>)) at the completion of the entire array of static steel to let the competitor know that they have neutralized all targets in the array.
...
- An RO directly observes a direct hit described by [Section 11.4.4.2](#id-11.4.4.2-falling-steel-targets) in real time and calls “Hit!”, informing the competitor that the target will be scored as a hit and they can move on.
- An RO directly observes a direct hit described by [Section 11.4.4.2](#id-11.4.4.2-falling-steel-targets) in real time on a falling steel target that is used to activate another. This is a Range Equipment Malfunction and the RO should use the “Stop!” command, and a re-shoot must be issued.

### 11.8.4 Painted falling steel

If a competitor impacts a painted falling steel target but the target does not fall, the hit shall be determined by [Section 11.4.4.2](#id-11.4.4.2-falling-steel-targets).
